...
cms<-cms[c(-1,-7,-8)]
names(cms)<-c('carrierA','carrierB','inpatient','outpatient','prescription')


#save files as RDS file to save memory:
saveRDS(beneficiary,'./dataset/beneficiary.RData')

for (i in 1:5){
  saveRDS(cms[[i]],paste0('./dataset/',names(cms)[i],'.RData'))
}
