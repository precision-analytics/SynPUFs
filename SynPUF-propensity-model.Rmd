...
```{r setup, echo = FALSE}
pacman::p_load(aws.s3, magrittr, dplyr, MatchIt, ggplot2, plotly, tableone, forcats, tidyr, knitr, kableExtra, gridExtra, tibble, stringr)

df = readRDS('propensity-data.RData')
dat = df %>% data.frame()
...
dat$statin = dat$level
dat$statin[dat$statin == 'low'] = '0'
dat$statin[dat$statin == 'high'] = '1'
dat$statin= as.numeric(dat$statin)


catVars = c('ckd', 'hypertension', 'hypercholesterol', 'heart.failure', 'injury.poison', 'hospital', 'lab.test')

dat %<>%
  mutate_at(catVars, funs(factor(.)), labels = c('No', 'Yes'))

dat$acute.kidney = dat$outcome
dat$acute.kidney = factor(dat$acute.kidney, labels = c('Readm. kidney', 'No readmission'))
...
dat  %<>% select(DESYNPUF_ID, sex, race, age_entry, ckd, hypertension, hypercholesterol, 
                 heart.failure, injury.poison, hospital, lab.test, statin, acute.kidney) %>% 
  na.omit
```
...
`r length(model)` were successfully matched with controls
...
prop.total = glm(statin ~ ckd + hypertension + hypercholesterol + heart.failure + 
                injury.poison + hospital + lab.test, family = binomial, data = dat)

prop.df = data.frame(pr_score = predict(prop.total, type = 'response'), 
                     statin = prop.total$model$statin)


pscore.plot.unmatched = ggplot(data = prop.df, aes(x = pr_score, fill = as.factor(statin))) + 
...
crude = glm(acute.kidney ~ statin, data = dat, family = binomial(link = 'logit')) %>% 
 ...
 adjusted = glm(acute.kidney ~ statin + ckd + hypertension + hypercholesterol + heart.failure + 
 ...
 model.compare = data.frame(rbind(crude, adjusted)) %>% 
         arrange(Variable)
```
...
ggplot(data = model.compare, aes(y = OR, colour = Model )) + 
        ...
        coord_flip() + 
        geom_pointrange(aes(x = Variable, y = OR, ymin = Lower.CI, 
                            ymax = Upper.CI), 
                        lwd = 1/2, position = position_dodge(width = 1/2), 
                        shape = 21, fill = 'WHITE') + 
        ggtitle('The association between statin use and readmission for acute kidney disease') + 
        labs(y = 'Odds Ratio (95% CI)', 
             x = ' ') + 
        scale_y_continuous(trans = "log", limits = c(0.1, 5), 
                           breaks = c(0.1, 0.3, 0.5, .75, 0, 1, 2, 3, 4, 5))
```
